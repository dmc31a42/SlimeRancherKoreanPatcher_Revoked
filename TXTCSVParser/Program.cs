...
namespace TXTCSVParser
{
    class Program
    {
        static void Main(string[] args)
        {
            if(args.Length < 3)
            {
                string help = "TXTCSVParser.exe -Process Filename1 FileName2 ...\r\n" +
                    "-Patch Original.txt Translated.csv\t : Patch Original text file using Translated csv(from Google)\r\n" +
                    "-Update Original.txt OldTranslated.csv NewTranslated.csv\t : Make Updated Translated.cvs using Origianl.txt and OldTranslated.csv\r\n" +
                    "-Diff Original.txt OldTranslated.csv DiffText.csv\t : Make Differential Text to DiffText.csv using Origianl.txt and OldTranslated.csv\r\n";
            ...
            switch (args[0])
            {
                case "-Patch":
                    {
                        string OrigianlTextFileName = args[1];
                        string TranslatedCSVFileName = args[2];
                        string OrigianlTextString = System.IO.File.ReadAllText(OrigianlTextFileName);
                        TXTParser txtParser = new TXTParser();
                        txtParser.Append(OrigianlTextString);
                        List<string[]> originalListContext = txtParser.ListContext;
                        string TranslatedCSVString = System.IO.File.ReadAllText(TranslatedCSVFileName);
                        ...
                        System.IO.File.WriteAllText(OrigianlTextFileName, PatchedString);
                ...
                case "-Update":
                    {
                        string OrigianlTextFileName = args[1];
                        string TranslatedCSVFileName = args[2];
                        string NewTranslatedCSVFileName = args[3];
                        string OrigianlTextString = System.IO.File.ReadAllText(OrigianlTextFileName);
                        TXTParser txtParser = new TXTParser();
                        txtParser.Append(OrigianlTextString);
                        List<string[]> originalListContext = txtParser.ListContext;
                ...
                case " - Diff":
                    break;
                default:
                    Console.WriteLine("Process Option Error");
                    return;
            }
        }
    }
